...
class FoxSellProductCard extends HTMLElement {
  ...
  updatePrice() {
    ...
    if (price > discountedPrice) {
      ...
      priceEl.innerHTML = `<span>${this.isAddOnCard && price === 0 ? 'Free' : window.foxsell?.formatMoney?.(price)}</span>`;
    }
  }
    ...
    return (`
      <foxsell-bundle-line-item data-item-id="${item.id}" data-category-id="${item.category.id}" data-category-title="${item.category.title}" data-quantity="${item.quantity}" class="foxsell-bundle-summary__item">
        ...
        <div>
          ...
          ${(item.category.id === '__add_ons__' || priceStrategy?.strategy !== 'fixed_pricing') ? `
          <div>
            ${itemPrice > discountedPrice ? `
              ...
              :
              `<div>
                <span>${item.category.id === '__add_ons__' && itemPrice === 0 ? 'Free' : window.foxsell?.formatMoney?.(itemPrice)}</span>
              </div>`
            }
          </div>` : ''}
        </div>
