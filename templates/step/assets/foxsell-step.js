...
class FoxSellProductModal extends HTMLElement {
  ...
  async renderProductModal(productId) {
    ...
    try {
      ...
      const response = await(await fetch(`/products/${productHandle}?sections=foxsell-step-product-modal`)).json();

      const parser = new DOMParser();
      const doc = parser.parseFromString(response['foxsell-step-product-modal'], 'text/html');
...
class StepProductCard extends FoxSellProductCard {
  constructor() {
    super();
...
class StepBundleSummary extends FoxSellBundleSummary {
      ...
      return `
        <div class="foxsell-bundle-summary__item-category">
          <div class="foxsell-bundle-summary__item-category-title">
            <span>${items[0].category.metadata.title}:</span>
            <span>${items[0].category.metadata.description}</span>
            <span class="foxsell-bundle-summary__item-category-quantity">${items[0].category.quantity}/${items[0].category.maxQuantity}</span>
          </div>
  ...
  renderLineItem(item) {
    ...
    return (`
      <foxsell-bundle-line-item data-item-id="${item.id}" data-category-id="${item.category.id}" data-category-title="${item.category.title}" data-quantity="${item.quantity}" class="foxsell-bundle-summary__item">
        <div class="foxsell-bundle-summary__item-image-wrapper">
          <img src="${itemImage}" class="foxsell-bundle-summary__item-image" />
...
class StepBundleProgressBar extends FoxSellBundleProgress {
  constructor() {
    super();
  }
  ...
  updateBundleProgress() {
    ...
    const priceWrapper = this.foxsell?.querySelector('.foxsell-bundle-summary__price-value');
    const { originalTotalPrice, totalPrice } = this.foxsell?.getTotalPrice() ?? { originalTotalPrice: 0, totalPrice: 0 };
    if (priceWrapper) {
      if (originalTotalPrice > 0 && totalPrice > 0 && (totalPrice !== originalTotalPrice)) {
...
class StepMixMatch extends FoxSellMixMatch {
  constructor() {
    ...
    this.previousButtons = this.querySelectorAll('.foxsell-category__previous-button');

    this.nextButtons = this.querySelectorAll('.foxsell-category__next-button');

    this.boundGoToPreviousStep = this.goToPreviousStep.bind(this);
    this.boundGoToNextStep = this.goToNextStep.bind(this);
  }
  ...
  connectedCallback() {
    this.validateBundle();
    if(this.previousButtons.length > 0) {
      this.previousButtons.forEach(button => button.addEventListener('click', this.boundGoToPreviousStep));
    }
    if(this.nextButtons.length > 0) {
      this.nextButtons.forEach(button => button.addEventListener('click', this.boundGoToNextStep));
    }
  }

  disconnectedCallback() {
    if(this.previousButtons.length > 0) {
      this.previousButtons.forEach(button => button.removeEventListener('click', this.boundGoToPreviousStep));
    }
    if(this.nextButtons.length > 0) {
      this.nextButtons.forEach(button => button.removeEventListener('click', this.boundGoToNextStep));
    }
  }
  ...
  validateStep() {
    ...
    const { currentCategory } = this.getStepProgress();
    if(currentCategory === null) return;

    const currentCategoryState = this.getCategory(currentCategory.getAttribute('data-category-id') ?? '');
    if(currentCategoryState === undefined) return;

    if(currentCategoryState.isMaxQuantity) {
      this.goToNextStep();
    }
  }
  ...
  getStepProgress() {
    ...
    const allCategories = this.querySelectorAll('.foxsell-category__item:not(.foxsell--hidden)');
    if(allCategories.length === 0) return { currentCategory: null, nextCategory: null, previousCategory: null };

    let currentCategory = null, nextCategory = null, previousCategory = null;

    for(let i = 0; i < allCategories.length; i++) {
      const category = allCategories[i];
      if(category && category.classList.contains('active')) {
        currentCategory = category;
        previousCategory = i > 0 ? allCategories[i - 1] ?? null : null;
        nextCategory = i < allCategories.length - 1 ? allCategories[i + 1] ?? null : null;
        break;
      }
    }

    if(currentCategory) {
      for(const step of this.querySelectorAll('.foxsell-mix-match__step')) {
        if(step.getAttribute('data-step') === currentCategory.getAttribute('data-category-id')) {
  ...
  goToNextStep() {
    const { nextCategory, currentCategory } = this.getStepProgress();
    if(nextCategory === null || currentCategory === null) return;
  ...
  goToPreviousStep() {
    const { previousCategory, currentCategory } = this.getStepProgress();
    if(previousCategory === null || currentCategory === null) return;
  ...
  updateStepButtons() {
    const { previousCategory, nextCategory } = this.getStepProgress();
    if(previousCategory === null) {
      this.previousButtons.forEach(button => button.setAttribute('disabled', 'disabled'));
    } else {
      this.previousButtons.forEach(button => button.removeAttribute('disabled'));
    }
  ...
  getAddOnsConfig() {
    ...
    if (!this.config || !this.config.addOnProducts || !this.config.stepByStepConfig) {
      return defaultConfig;
    }

    const addOnStrategy = this.config.stepByStepConfig?.add_on_settings.strategy ?? '';

    const allowedIds = this.config.stepByStepConfig?.add_on_settings.variants.map(
      (variant) => variant.id
    ) ?? [];

    const minimum = this.config.settings.addOn?.minimum ?? 0;
    const maximum = this.config.settings.addOn?.maximum ?? 0;

    const selectedQuantity = this.getSelectedAddOns().reduce((sum, item) => sum + item.quantity, 0);
    const isMaxedOut = selectedQuantity >= maximum;
  ...
  autoAddAddOns() {
    ...
    this.config?.stepByStepConfig?.add_on_settings.variants.forEach(variant=> {
