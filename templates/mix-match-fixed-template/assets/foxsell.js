class FoxSellMixMatch extends HTMLElement {
  ...
  handleAddToCart(event) {
    ...
    const uniqueBundleId = this.bundleId + '_' + Date.now();
    ...
    let body = {
      'items': [{
        ...
        'quantity': 1,
        'properties': {
          '__foxsell:dynamic_add_on_bundle_id': uniqueBundleId,
          '__foxsell:dynamic_add_on_bundle_items': JSON.stringify(bundleLineItems)
        }
       }]
     };
...
class FoxSellProductOptions extends HTMLElement {
  ...
  updateAddToCartButtonState() {
    ...
    const addToCartButton = mixMatchComponent.querySelector('button[name="foxsell-mix-match-add-to-cart"]');
